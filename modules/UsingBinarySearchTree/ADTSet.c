...
//επιστρέφει το size του υποδέντρου του κόμβου node

static int node_size(SetNode node){
	return node ? node->sub_size : 0;
}

//Ενημερώνουν το size του κόμβου node
//++
static void node_inc_size(SetNode node) {
    if (node) {
        node->sub_size = 1 + node_size(node->left) + node_size(node->right);
    }
}

//--
static void node_dec_size(SetNode node) {
    if (node) {
        node->sub_size =node_size(node->left) + node_size(node->right)-1;
    }
}
...
static SetNode node_insert(SetNode node, CompareFunc compare, Pointer value, bool* inserted, Pointer* old_value) {
	...
	if (compare_res == 0) {
		...
		*old_value = node->value;
		...
		if(*inserted){
		//Ενημερώνουμε το νέο size του κόμβου
		node_inc_size(node);
		}
	...
	if (compare_res == 0) {
		...
		*old_value = node->value;
		...
		if (node->left == NULL) {
			...
			min_right->sub_size= node->sub_size -1;
			free(node);
